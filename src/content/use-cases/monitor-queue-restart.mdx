---
title: "Monitoring queue:restart in Laravel Production"
description: "Monitor Laravel's queue:restart command in production with instant alerts for failures, missed runs, and slow execution using Crontinel."
date: 2026-04-07
---

Running `queue:restart` in production seems harmless until you realize it does not actually restart your workers. It sets a flag in cache that tells each worker to terminate after finishing its current job. If your cache driver is misconfigured, if a worker is stuck on a long-running job, or if your process manager fails to bring workers back, you end up with zero active workers and a queue that silently fills up.

This is not a hypothetical. It happens when teams deploy new code and run `php artisan queue:restart` as part of their deployment script, assuming workers will bounce back immediately. Sometimes they do. Sometimes they don't.
